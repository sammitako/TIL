...
- $x$의 변화: $Δx$가 $f(x)$를 얼마만큼 변화시키는 지: $f(x+Δx) - f(x)$를 나타냄
...
- **중앙차분**
  ...
  : $x$를 기준으로 $Δx$를 앞과 뒤, 2개 잡으면 함수값이 3개가 생기게 된다.
...
$f(x,y)$를 $x$에 대해서 partial derivative(편미분)하는 경우
...
$Δx$: 0.00001 = $10^{-6}$ 정도로 정의 (floating point 오류 고려)
...
입력변수가 2개이기 때문에 $x$와 $y$에 대해 각각 편미분 시행
